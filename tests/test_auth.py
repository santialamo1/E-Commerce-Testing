...
def test_login_succesful(base_url):
    response = requests.post(f"{base_url}/auth/login", json={"username": "mor_2314", "password": "83r5^_"})
    user = response.json()
...
def test_login_missing_password(base_url):
    response = requests.post(f"{base_url}/auth/login", json={"username": "mor_2314", "password": ""})
